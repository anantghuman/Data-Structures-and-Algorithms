...
public class MinHeapGeneric<T extends Comparable<T>>{
    ...
    public MinHeapGeneric(T... vals){
        heap = (T[]) new Comparable[vals.length];
    ...
    private void siftDown(int index){
        ...
        else if(heap[right].compareTo(heap[left]) > 0 && heap[index].compareTo(heap[left]) > 0){
            T temp = heap[index];
            heap[index] = heap[left];
            heap[left] = temp;
            siftDown(left);
        }
        else if(heap[left].compareTo(heap[right]) > 0 && heap[right].compareTo(heap[index]) > 0) {
            T temp = heap[index];
